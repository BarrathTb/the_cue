const List<Map<String, String>> songs = [
  {
    'image': 'assets/images/playlist_card_one.png',
    'title': 'Blinding Lights',
    'artist': 'The Weeknd'
  },
  {
    'image': 'assets/images/playlist_card_two.png',
    'title': 'Save Your Tears',
    'artist': 'The Weeknd & Ariana Grande'
  },
  {
    'image': 'assets/images/playlist_card_three.png',
    'title': 'Good 4 U',
    'artist': 'Olivia Rodrigo'
  },
  {
    'image': 'assets/images/playlist_card_four.png',
    'title': 'Peaches',
    'artist': 'Justin Bieber, Daniel Caesar & Giveon'
  },
  {
    'image': 'assets/images/playlist_card_five.png',
    'title': 'Stay',
    'artist': 'The Kid LAROI & Justin Bieber'
  },
  {
    'image': 'assets/images/card_image_one.png',
    'title': 'Levitating',
    'artist': 'Dua Lipa Featuring DaBaby'
  },
  {
    'image': 'assets/images/card_image_two.png',
    'title': 'Kiss Me More',
    'artist': 'Doja Cat Featuring SZA'
  },
  {
    'image': 'assets/images/card_image_three.png',
    'title': 'Astronaut In The Ocean',
    'artist': 'Masked Wolf'
  },
  {
    'image': 'assets/images/card_image_four.png',
    'title': 'Montero (Call Me By Your Name)',
    'artist': 'Lil Nas X'
  },
  {
    'image': 'assets/images/card_image_five.png',
    'title': 'Rapstar',
    'artist': 'Polo G'
  },
  {
    'image': 'assets/images/card_image_six.png',
    'title': 'Leave The Door Open',
    'artist': 'Silk Sonic (Bruno Mars & Anderson .Paak)'
  },
  {
    'image': 'assets/images/the_cue_cali.png',
    'title': 'Drivers License',
    'artist': 'Olivia Rodrigo'
  },
  // {
  //   'image': 'assets/images/playlist_card_six.png',
  //   'title': 'Heartbreak Anniversary',
  //   'artist': 'Giveon'
  // },
  // {
  //   'image': 'assets/images/playlist_card_seven.png',
  //   'title': 'Without You',
  //   'artist': 'The Kid LAROI'
  // },
  // {
  //   'image': 'assets/images/playlist_card_eight.png',
  //   'title': 'Telepatia',
  //   'artist': 'Kali Uchis'
  // },
  // {
  //   'image': 'assets/images/playlist_card_nine.png',
  //   'title': 'Deja Vu',
  //   'artist': 'Olivia Rodrigo'
  // },
  // {
  //   'image': 'assets/images/playlist_card_ten.png',
  //   'title': 'Best Friend',
  //   'artist': 'Saweetie Featuring Doja Cat'
  // },
  // {
  //   'image': 'assets/images/card_image_seven.png',
  //   'title': 'Beautiful Mistakes',
  //   'artist': 'Maroon 5 & Megan Thee Stallion'
  // },
  // {
  //   'image': 'assets/images/card_image_eight.png',
  //   'title': 'What’s Next',
  //   'artist': 'Drake'
  // },
  // {
  //   'image': 'assets/images/card_image_nine.png',
  //   'title': 'My Ex’s Best Friend',
  //   'artist': 'Machine Gun Kelly x blackbear'
  // },
  // {
  //   'image': 'assets/images/card_image_ten.png',
  //   'title': '34+35',
  //   'artist': 'Ariana Grande'
  // },
  // {
  //   'image': 'assets/images/card_image_eleven.png',
  //   'title': 'Calling My Phone',
  //   'artist': 'Lil Tjay Featuring 6LACK'
  // },
  // {
  //   'image': 'assets/images/card_image_twelve.png',
  //   'title': 'Up',
  //   'artist': 'Cardi B'
  // },
  // {
  //   'image': 'assets/images/card_image_thirteen.png',
  //   'title': 'Heat Waves',
  //   'artist': 'Glass Animals'
  // },
  // {
  //   'image': 'assets/images/card_image_fourteen.png',
  //   'title': 'Go Crazy',
  //   'artist': 'Chris Brown & Young Thug'
  // },
  // {
  //   'image': 'assets/images/card_image_fifteen.png',
  //   'title': 'Mood',
  //   'artist': '24kGoldn Featuring iann dior'
  // },
  // {
  //   'image': 'assets/images/card_image_sixteen.png',
  //   'title': 'Therefore I Am',
  //   'artist': 'Billie Eilish'
  // },
  // {
  //   'image': 'assets/images/card_image_seventeen.png',
  //   'title': 'Savage Love (Laxed - Siren Beat)',
  //   'artist': 'Jawsh 685 x Jason Derulo'
  // },
  // {
  //   'image': 'assets/images/card_image_eighteen.png',
  //   'title': 'positions',
  //   'artist': 'Ariana Grande'
  // },
  // {
  //   'image': 'assets/images/card_image_nineteen.png',
  //   'title': 'Holy',
  //   'artist': 'Justin Bieber Featuring Chance The Rapper'
  // },
  // {
  //   'image': 'assets/images/card_image_twenty.png',
  //   'title': 'Laugh Now Cry Later',
  //   'artist': 'Drake Featuring Lil Durk'
  // },
  // {
  //   'image': 'assets/images/card_image_twentyone.png',
  //   'title': 'Dakiti',
  //   'artist': 'Bad Bunny & Jhay Cortez'
  // }

  // Add all your songs here.
];